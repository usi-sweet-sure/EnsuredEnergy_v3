...
public partial class BuildButton : TextureButton {
	...
	public override void _Ready() {
		...
		BuildSprite = GetNode<Sprite>("BuildingInfo/Building");
		PlantSprite = GetNode<Sprite>("BuildingInfo/Planting");
		...
		Plate = GetNode<Sprite>("BuildingInfo/Building/Plate");
		...
		//!!! Question ShowBuildMenu += BM._OnShowBuildMenu;
		Connect(nameof(ShowBuildMenuEventHandler), this, nameof(BM._OnShowBuildMenu));
		Cancel.Connect("pressed", this, nameof(_OnCancelPressed));
		BuildInfoB.Connect("pressed", this, nameof(_OnBuildInfoPressed));

		// Make sure that the location is set correctly
		if(AllowHydro) {
			BL = new BuildLocation(RectPosition, Building.Type.HYDRO, Building.Type.RIVER);
		} else {
			BL = new BuildLocation(RectPosition, Building.Type.GAS, Building.Type.SOLAR, Building.Type.TREE, Building.Type.WIND, Building.Type.WASTE, Building.Type.BIOMASS);
		}

		// Connect the button press callback

		//!!! Question Pressed += _OnPressed;
		Connect("pressed", this, nameof(_OnPressed));
		//!!! Question MouseEntered += _OnHover;
		Connect("mouse_entered", this, nameof(_OnHover));
	}
	...
	private void FinishBuild() {
		...
		EmitSignal(nameof(BuildDoneEventHandler));
	}
	...
	private void UpdatePowerPlant(ref PowerPlant PP, PowerPlant PPRec) {
		...
		EmitSignal(nameof(UpdateBuildSlotEventHandler), this, PP, false);
	}
	...
	public void _OnPressed() {
		...
		if(BS == BuildState.IDLE) {
			BM.Show();
			EmitSignal(nameof(ShowBuildMenuEventHandler), this);
		}
	}
