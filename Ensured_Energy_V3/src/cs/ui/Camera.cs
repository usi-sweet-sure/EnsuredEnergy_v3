...
public partial class Camera : Camera2D {
	...
	private Vector2 ZOOM_MAX = new (3f,3f);
...
//	}
	...
	public override void _UnhandledInput(InputEvent E) {
		...
		if(E is InputEventMagnifyGesture Magnify) {
			...
			TargetZoom.x = Mathf.Clamp(zoom_factor.x, ZOOM_MIN.x, ZOOM_MAX.x);
			TargetZoom.y = Mathf.Clamp(zoom_factor.y, ZOOM_MIN.y, ZOOM_MAX.y);
			Zoom = TargetZoom;
		...
		if(E is InputEventPanGesture Pan) {
			...
			zoom_factor.x += Pan.Delta.x * zoom_sensitivity;
			TargetZoom.x = Mathf.Clamp(zoom_factor.x, ZOOM_MIN.x, ZOOM_MAX.x);
			TargetZoom.y = Mathf.Clamp(zoom_factor.y, ZOOM_MIN.y, ZOOM_MAX.y);
			Zoom = TargetZoom;
			//ScalePlants(TargetZoom);
		...
		if(E is InputEventMouseMotion MouseMotion && pressed) {
		...
		if(E is InputEventMouseButton MouseBtn) {
			...
			if (MouseBtn.ButtonIndex == (int)(ButtonList.WheelDown)) {
					...
					if(Zoom > ZOOM_MIN) {
						...
						ZoomVal.x = Mathf.Clamp(NewZoom.x, ZOOM_MIN.x, ZOOM_MAX.x);
						ZoomVal.y = Mathf.Clamp(NewZoom.y, ZOOM_MIN.y, ZOOM_MAX.y);
						Zoom = ZoomVal;
						Tween TweenZoomIn = new Tween();
						TweenZoomIn.InterpolateProperty(this, "zoom", Zoom, ZoomVal, 0.3f);
						//ScalePlants(ZoomVal);
					}
			}
			...
			if (MouseBtn.ButtonIndex == (int)(ButtonList.WheelUp)) {
					...
					if(Zoom < ZOOM_MAX) {
						...
						ZoomVal.y = Mathf.Clamp(NewZoom.y, ZOOM_MIN.y, ZOOM_MAX.y);
						Zoom = ZoomVal;
						Tween TweenZoomOut = new Tween();
						TweenZoomOut.InterpolateProperty(this, "zoom", Zoom, ZoomVal, 0.3f);
						//ScalePlants(ZoomVal);
					}
			}
		}
